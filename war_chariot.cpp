/*
  ...
  Task -
  Students are expected to submit the Group assignment on Sun 22nd May by mid-night as announced earlier. 
  Only one submission per group to be uploaded on the classroom. You are expected to submit along with your code,
  document giving any specific instructions to compile and run the code. Also submit a video clip (not more than 2 minutes)
  demonstrating all features of your application with voice over explaining the features implemented. There shall be no viva
  and the evaluation will be based on the submission only.
 */
...
void draw_chariot(float x, float y, float z)
{
    ...
    // BASE OF CHARIOT
    glPushMatrix();
    float color[] = { 0.81, 0.71, 0.23 };   // brown color rgb(0.81,0.71,0.23)
    float color1[] = { 0.61, 0.51, 0.03 }; // dark brown color rgb(0.61,0.51,0.03)
    float color2[] = { 0.1, 0.1, 0.1 };   // black color rgb(0.1,0.1,0.1)
    float color3[] = { 0.35, 0.16, 0.14 }; // dark red color rgb(0.35,0.16,0.14)
